...
class TransformerModel(nn.Module):
    """Transformer模型（性能优化版）"""
    def __init__(self, input_size, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, output_size=1, dropout=0.1):
...
class StockPredictor:
    ...
    def train_model(self, model_name, model, X_train, y_train, X_val, y_val, epochs=50, batch_size=32, lr=0.001, early_stopping_patience=10, verbose=True, progress_callback=None, grad_accumulation_steps=1, use_amp=False):
        """训练模型（优化版）"""
        # 数据加载器优化
    ...
    def train_arima_model(self, model_name, X_train, y_train, X_test, y_test, p=1, d=0, q=1):
        ...
        # 计算ARIMA的损失历史
        arima_history = {'train_loss': [], 'val_loss': []}
        
        # 计算训练损失
        train_pred = model_fit.fittedvalues
        train_loss = mean_squared_error(train_data, train_pred)
        arima_history['train_loss'] = [train_loss] * 20  # 为了与其他模型的epoch数匹配
        ...
        for i in range(len(test_data)):
            ...
            # 计算当前验证损失
            if i > 0:
                val_loss = mean_squared_error(test_data[:i], predictions[:i])
        ...
        self.histories[model_name] = arima_history
        
        return metrics, predictions, test_data, arima_history
